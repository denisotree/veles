"""Turn execution for the inline REPL.

The per-turn machinery: the behavioural system-prompt block, the turn
system-prompt assembler, the post-turn learning-loop hooks, state
carry-forward, the fallback simple-loop turn driver, the `/command`
dispatcher, and `TurnMixin` (the `_ReplApp` turn methods). Rendering the
streamed answer lives in `render.py`.
"""
...
def _handle_slash(
    ...
    ctx = SlashContext(state=state, project=project, store=store, errors=errors)
