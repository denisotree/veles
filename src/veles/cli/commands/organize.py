...
from the active layout pack's `organize` skill (see
`veles.modules.organize.dispatcher`), so the behaviour is layout-specific by
construction. The module owns the dispatcher and the `move_file` primitive;
this command builds and runs the agent, so the module never imports the CLI.
"""
