...
def test_suggest_prints_clusters_without_save(project_with_cluster, capsys) -> None:
    ...
    proposals_dir = project_with_cluster.wiki_root / "wiki" / "proposals"
...
def test_suggest_with_save_persists_proposals(project_with_cluster, capsys) -> None:
    ...
    out = capsys.readouterr().out
    assert "wrote" in out
    pages = list((project_with_cluster.wiki_root / "wiki" / "proposals").iterdir())
    assert pages, "expected at least one proposal markdown file"
    # LOG.md should mention the proposal op
    log = (project_with_cluster.wiki_root / "LOG.md").read_text(encoding="utf-8")
...
def test_proposals_block_skipped_when_stale(project_with_cluster) -> None:
    ...
    page = project_with_cluster.wiki_root / "wiki" / "proposals" / "stale.md"
...
def test_recent_proposals_used_by_runtime(project_with_cluster) -> None:
    ...
    # same wiki/proposals/ pages as the rest of the M62 surface.
