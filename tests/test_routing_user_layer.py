...
def test_user_provider_base_layer(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    project = init_project(tmp_path / "p", name="p")
    home = tmp_path / "home"
...
def test_project_scope_beats_user_scope(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    project = init_project(tmp_path / "p", name="p")
    save_project_config(
...
def test_embedding_never_inherits_any_base(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    project = init_project(tmp_path / "p", name="p")
    save_project_config(
...
def test_embedding_explicit_route_honored(tmp_path: Path) -> None:
    project = init_project(tmp_path / "p", name="p")
    save_project_config(
        project, {"routing": {"tasks": {"embedding": "ollama:nomic-embed-text"}}}
    )
