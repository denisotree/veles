...
def test_empty_response_returns_empty_stopped_reason() -> None:
    ...
    stopped_reason='empty', not 'completed'. M214 (B2) forces a tool-free answer
    round first; M254 raised that from a fixed one retry to
    `_EMPTY_ANSWER_NUDGE_LIMIT`, because one was not enough for a model that
    finishes and says nothing. Only after the limit does it finalize 'empty'."""
    from veles.core.agent import _EMPTY_ANSWER_NUDGE_LIMIT

    rounds = _EMPTY_ANSWER_NUDGE_LIMIT + 1
    provider = _StubProvider(responses=[_final("")] * rounds)
    agent = Agent(provider, Registry(), model="m", max_iterations=rounds + 1)
    result = agent.run("hi")
    assert result.text == ""
...
def test_run_result_carries_invoked_tool_names() -> None:
    ...
    empty round is nudged for a closing answer (M254: up to
    `_EMPTY_ANSWER_NUDGE_LIMIT` times); if it stays empty the run finalizes
    'empty' with the tool work still recorded."""
    from veles.core.agent import _EMPTY_ANSWER_NUDGE_LIMIT

    reg = _echo_registry()
    empties = [_final("")] * (_EMPTY_ANSWER_NUDGE_LIMIT + 1)
    provider = _StubProvider(responses=[_tool_call("echo"), *empties])
    agent = Agent(provider, reg, model="m", max_iterations=len(empties) + 2)
    result = agent.run("go")
    assert result.stopped_reason == "empty"
