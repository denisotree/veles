...
def test_sanitize_uses_current_project_when_unspecified(monkeypatch, tmp_path: Path) -> None:
...
def test_sanitize_with_explicit_project_overrides_context(monkeypatch, tmp_path: Path) -> None:
    _isolate_home(monkeypatch, tmp_path)
    ...
    try:
        out = sanitize(f"see {explicit_project.root.resolve()}", project=explicit_project)
    finally:
        reset_active_project(token)
...
def test_sanitize_runs_builtin_chain_in_one_call(monkeypatch, tmp_path: Path) -> None:
    ...
    msg = f"path={project.root.resolve()} home={fake_home} user=alicebob key=sk-{'a' * 40}"
