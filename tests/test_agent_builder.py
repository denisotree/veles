...
(c) the monkeypatch contract — patching a helper on its owning module IS
    picked up by the factory at call time.
"""
...
def test_check_api_key_false_skips_gate(project, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "veles.cli._console.ensure_api_key",
...
def test_monkeypatch_contract_is_lazy(project, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    at module-import time — yet a later patch of `provider_factory.make_provider`
