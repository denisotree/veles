...
Veles 刻意保持 **极简且清晰解耦** —— 模块职责单一，不存在巨型文件。它是 **本地优先** 的：你针对机器上的某个目录运行它，它会在那里保存自己的结构化记忆。

## 五大支柱（核心）

核心中的一切都服务于以下五项工作之一：

1. **项目记忆** —— 一份结构化产物（独立于你的内容），保存会话日志、习得的规则/洞见、项目文件映射，以及带遥测数据的技能/工具注册表。参阅 [项目记忆与学习循环](project-memory-and-learning-loop.md)。
2. **学习循环** —— 策展器（curator）、洞见提取器以及"做梦"机制，它们让记忆保持新鲜，并将经验转化为可复用的规则。
...
一个智能体循环服务于多个项目。每个项目拥有自己的目录，包含各自的上下文和记忆；`AGENTS.md` 通过符号链接指向 `CLAUDE.md`/`GEMINI.md`，因此在该目录下启动的外部 CLI 能看到相同的上下文。参阅 [多项目](../how-to/multi-project-and-subprojects.md)。
