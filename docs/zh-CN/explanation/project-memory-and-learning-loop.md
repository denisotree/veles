...
### 策展器 —— 整合会话
策展器将较旧的会话提炼为持久记忆：始终生成 SQL 洞见和规则；此外，当项目的布局启用了 wiki 引擎时，还会生成一个 `wiki/sessions/` 页面。它在空闲/轮后定时器上运行，或通过 `veles curate` 按需运行。

### 做梦 —— 后台维护
`veles dream`（以及守护进程在空闲时）会提取洞见、对技能和洞见去重、建议晋升，并（在 wiki 布局下）对 wiki 进行 lint —— 在不阻塞你的前提下让记忆保持新鲜。加上 `--include-consolidation` 可进行更深入的 LLM 处理。
...
由于记忆是结构化的，且循环持续运行，一个 Veles 项目 **用得越多就越有用** —— 它会学习你的约定、避免重复错误，并将答案根植于它实际见过的内容之上。
