...
| `VELES_HOME` | — | `VELES_USER_HOME` 的旧版别名 |
...
| `VELES_CONFIG_FILENAME` | `config.toml` | 覆盖配置文件名（用于测试） |
...
| `VELES_MEMORY_RERANK` | 开启 | 对内存召回进行向量重排序（`0`/`false` 禁用） |
...
`VELES_BUNDLE_VERSION`、`VELES_CACHE_BREAKPOINT` — 内部使用；你应该不需要设置它们。
