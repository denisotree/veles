...
**除非你主动要求，否则不会删除任何内容。** `turn_retention_days` 默认为 `0`，
即永久保留全部对话轮次。若想给 `memory.db` 设上限，请填入天数：

```toml
[memory]
turn_retention_days = 90   # 0（默认）保留全部
```

启用后，早于该天数的原始对话轮次会被删除；而从中提取出的**洞察**和规则无论如何都
永久保留。记录是原材料，洞察才是阅读它的目的。

一份记录被丢弃需要**同时**满足两个条件：早于保留窗口，**并且**策展器已经处理过该
会话。策展器尚未触及的会话永远不会被删除，无论多旧——否则记录会在还没从中学到任何
东西之前就被销毁。

启用它的代价：`veles sessions search` 只能找到窗口之内的文本。`veles sessions list`
