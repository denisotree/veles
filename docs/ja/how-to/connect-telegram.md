...
`/mode` はチャットのエージェントモードを切り替えます：`default`（切り替え前と同じく
エージェントが直接答える）、`auto`（メッセージごとに先に計画するかを判断）、
`planning`（計画のみで何も変更しない）、`writing`（ツールを使って実行する）。
現在のモードにはチェックが付きます。選択はデーモンの再起動まで有効です。
モードの状態行（例：*auto → plan*）は回答の上に表示されます。

`/goal <タスク>` はチャット内で目標を実行します。エージェントは必要なことを質問し、
これから従う計画を示します。`yes` と返すと自分で作業を進め、目標が達成されるか
予算が尽きるまで、各ステップの後に1行ずつ送ります。承認は引き続きボタンで届きます。
`/goal` は進捗を表示し、`/goal cancel` は現在のステップの後で停止し、
`/goal resume` は停止した目標を再開します。

あなたにしか分からない情報が必要なとき、エージェントはチャットで質問します。
提示された回答をタップするか、自分で入力してください。5分以内に回答がなければ、
最善の仮定で進め、何を仮定したかを伝えます。
