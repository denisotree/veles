...
**頼まない限り何も削除されません。** `turn_retention_days` の既定値は `0` で、
会話ターンをすべて永久に保持します。`memory.db` に上限を設けたい場合は日数を
指定してください：

```toml
[memory]
turn_retention_days = 90   # 0（既定）はすべて保持
```

有効にすると、その日数より古い生の会話ターンは削除されます。そこから抽出された
**インサイト**とルールは、設定にかかわらず永久に保持されます。会話記録は原材料で
あり、インサイトはそれを読んだ目的です。
...
有効にした場合の代償：`veles sessions search` は保持期間内のテキストしか見つけられ
ません。`veles sessions list` は古い実行も表示し続けます。セッション行（id・
タイトル・タイムスタンプ）は残り、消えるのはメッセージ本文だけだからです。削除は
