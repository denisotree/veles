...
يشمل `VELES_LOCAL_JSON_MODE` فقط الاستدعاءات التي يعرف Veles أنها تتوقّع JSON صارمًا: اليوم
هو advisor (يستخدمه `advisor_review` ومسار verify ومرحلة CHECK في وضع goal). أما أدوار الوكيل
العادية فلا تحصل عليه أبدًا: مسار fenced-tools يحتاج نصًّا حول كتله، وقيد كائن JSON يمنع ذلك.
كما أنه يُصلح نفسه — أي خادم يرفض هذا المعامل يُعطَّل لديه لبقية العملية ويُعاد الطلب بدونه،
لذلك لا تحتاج عادةً إلى هذا المفتاح.
