...
| العَلَم | الافتراضي | الغرض |
|---|---|---|
| `--model <id>` | محلول من نموذج `[engine]` للمشروع → `default_model` للمستخدم (لا افتراضي مُضمَّن) | معرّف النموذج |
| `--provider <name>` | `openrouter` | المزوّد (انظر أدناه) |
| `--max-tokens-total <n>` | `100000` | ميزانية الرموز التراكمية؛ يُعطِّلها `0` |
| `--max-iterations <n>` | `1000` | الحدّ الأقصى لتكرارات استدعاء الأدوات لكل دور |
