...
```toml
[memory]
turn_retention_days = 90   # 0 يحفظ كل شيء إلى الأبد
```

تُحذف أدوار المحادثة الخام بعد هذا العدد من الأيام، بينما تُحفظ إلى الأبد
**الاستنتاجات** والقواعد المستخلَصة منها. النص الخام هو المادة الأولية،
والاستنتاجات هي الغاية التي قُرئ من أجلها — وهكذا يتوقف `memory.db` عن النمو بلا
حدّ بينما يحتفظ الوكيل بما تعلّمه.
...
الكلفة الظاهرة: لا يجد `veles sessions search` إلا النص الواقع داخل النافذة. أما
