...
## أوضاع الوكيل في المحادثة

يبدّل `/mode` وضع الوكيل في المحادثة: `default` (يجيب الوكيل مباشرة، كما قبل أي
تبديل)، و`auto` (يقرر لكل رسالة هل يخطط أولًا)، و`planning` (يخطط فقط ولا يغيّر
شيئًا)، و`writing` (ينفّذ بأدواته). يُعلَّم الوضع الحالي بعلامة. يبقى الاختيار
ساريًا حتى إعادة تشغيل الخدمة. يظهر سطر حالة الوضع، مثل *auto → plan*، فوق
الإجابة.

يشغّل `/goal <مهمة>` هدفًا داخل المحادثة. يسأل الوكيل عمّا يحتاج إلى معرفته
ويعرض الخطة التي سيتبعها. عندما ترد بـ `yes` يعمل بمفرده ويرسل سطرًا بعد كل خطوة
حتى يتحقق الهدف أو تنفد الميزانية. تستمر طلبات الموافقة في الوصول كأزرار. يعرض
`/goal` التقدّم، ويوقفه `/goal cancel` بعد الخطوة الحالية، ويستأنف `/goal resume`
هدفًا متوقفًا.

عندما يحتاج الوكيل إلى تفصيل لا يعرفه غيرك، يسأل في المحادثة. اضغط أحد الأجوبة
المقترحة أو اكتب جوابك. إن لم تجب خلال خمس دقائق، يتابع على أفضل افتراض ويذكر ما
افترضه.
