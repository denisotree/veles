...
> 🌐 **언어:** **English** · [Русский](../../ru/reference/environment-variables.md)

Veles는 런타임에 이 변수들을 읽습니다. API 키와 토큰은 OS 키체인(`veles secret set …`)에 저장하는 것이 가장 좋습니다. 환경 변수는 폴백이자 재정의 수단입니다.
...
| `ANTHROPIC_API_KEY` | anthropic | Anthropic API 직접 접속 |
| `OPENAI_API_KEY` | openai | OpenAI API 직접 접속 |
| `GEMINI_API_KEY` | gemini | Google Gemini 기본 키 |
| `GOOGLE_API_KEY` | gemini | Google Gemini 폴백 키 |

`claude-cli`와 `gemini-cli`는 각자의 바이너리를 통해 인증하므로 환경 변수가 필요 없습니다.
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `VELES_USER_HOME` | `~` | `~/.veles/`(상태, 캐시, 키체인 인덱스)를 담는 홈 디렉터리 재정의 |
| `VELES_HOME` | — | `VELES_USER_HOME`의 레거시 별칭 |
| `VELES_REGISTRY_PATH` | `~/.veles/…` | 멀티 프로젝트 레지스트리 경로 재정의 |
| `VELES_LOCALE` | `[user] language` 또는 `en` | 이번 실행의 UI 로케일 재정의 |
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `VELES_NO_WIZARD` | off | 최초 실행 마법사 건너뜀(TTY도 필요) |
| `VELES_MANAGER_MODE` | off | `veles run`에 멀티 에이전트 매니저 강제 적용(`1`로 활성화 / `0`으로 강제 비활성화) |
| `VELES_FENCED_TOOLS` | off | 펜스/샌드박스 실행 경로에서 도구 실행 |
| `VELES_TRUST_AUTO_ALLOW` | off | 신뢰 계층 우회(CI / 자동 조종 / 사전 승인된 서브 에이전트) |
| `VELES_SANDBOX_ROOTS` | 프로젝트 + `~/.veles` | 읽기/쓰기 샌드박스 루트의 `:`로 구분된 재정의 |
| `VELES_FETCH_ALLOW_PRIVATE` | off | 도구가 RFC-1918 / 사설 주소를 요청하도록 허용 |
| `VELES_MEMORY_RERANK` | on | 메모리 리콜의 벡터 재순위(`0`/`false`로 비활성화) |
...
`VELES_BUNDLE_VERSION`, `VELES_CACHE_BREAKPOINT` — 내부 변수로, 직접 설정할 필요는 없습니다.
