...
| `VELES_LOCAL_JSON_MODE` | вкл | Отправлять `response_format: json_object` в локальных вызовах, которые обязаны вернуть JSON-объект (`0` — выключить) |

`VELES_LOCAL_JSON_MODE` затрагивает только вызовы, про которые Veles знает, что они ждут
строгий JSON: сегодня это advisor (используется в `advisor_review`, в проверке verify и в
фазе CHECK режима goal). Обычные ходы агента его не получают: путь fenced-tools требует
прозы вокруг своих блоков, а ограничение на JSON-объект её запрещает. Механизм ещё и
самовосстанавливается — бэкенд, отвергнувший параметр, отключает его до конца процесса, и
запрос повторяется без него, так что переключатель обычно не нужен.
