...
```toml
[memory]
turn_retention_days = 90   # 0 — хранить всё вечно
```

Сырые ходы разговора удаляются по истечении этого срока, а **инсайты** и
правила, извлечённые из них, хранятся вечно. Транскрипт — сырьё, инсайты — то,
ради чего его читали, поэтому `memory.db` перестаёт расти без ограничения, а
агент сохраняет выученное.
...
Видимая цена: `veles sessions search` находит текст только внутри окна.
