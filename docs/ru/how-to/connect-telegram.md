...
## Режимы агента в чате

`/mode` переключает режим агента в чате: `default` (агент отвечает сразу, как до
любого переключения), `auto` (для каждого сообщения решает, нужен ли сначала
план), `planning` (только планирует, ничего не меняет) и `writing` (действует
своими инструментами). Текущий режим отмечен галочкой. Выбор действует до
перезапуска демона. Строка состояния режима, например *auto → plan*, выводится
над ответом.

`/goal <задача>` запускает цель прямо в чате. Агент уточняет, что ему нужно
знать, и показывает план. После вашего `yes` он работает сам и присылает строку
после каждого шага, пока цель не будет достигнута или не кончится бюджет.
Запросы одобрения по-прежнему приходят кнопками. `/goal` показывает ход цели,
`/goal cancel` останавливает её после текущего шага, `/goal resume` продолжает
остановленную.

Когда агенту нужна деталь, которую знаете только вы, он спрашивает в чате.
Нажмите один из предложенных ответов или напишите свой. Если ответа нет пять
минут, агент продолжает на лучшем допущении и сообщает, что именно допустил.
