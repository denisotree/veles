...
> 🌐 **Языки:** [English](../../en/how-to/run-as-daemon.md) · **Русский**

Демон — это опциональный долгоживущий HTTP+WS сервер, который предоставляет агента
в виде API — основа для [каналов](connect-telegram.md) (Telegram, …),
...
`start` отсоединяется и возвращает вам shell. Для процесса на переднем плане
...
Модель и провайдер демона берутся из конфигурации проекта и **фиксированы на всё
время его работы** — задайте их перед запуском:
...
API-клиенты аутентифицируются по bearer-токену:
...
`d` удалить; `c`/`x` добавить/убрать канал; `q` выход.
...
У каждой именованной сессии есть свой блок конфигурации `[daemon.<name>]` и свои
каналы (`[daemon.<name>.channels.*]`).
...
- [Подключить канал Telegram](connect-telegram.md)
- [Запланировать задачи](long-running-tasks.md)
