...
**モノリシックではなく、モジュラー** — 最小限のコア(メモリ、エージェントループ、プロバイダープロトコル、ツールレジストリ)。それ以外のすべて — デーモン、Telegram ゲートウェイ、ディープリサーチ、ジョブスケジューラー — はオプションのロード可能なモジュールです。
...
| `Ctrl+X Ctrl+E` | 現在の下書きを `$EDITOR` で開く |
| `Tab` | スラッシュコマンドの自動補完 |
| `Ctrl+D` | 終了 |
