...
**Модульность, а не монолит** — минимальное ядро (память, цикл агента, протокол провайдера, реестр инструментов). Всё остальное — демон, шлюз Telegram, глубокое исследование, планировщик задач — это опциональный загружаемый модуль.
...
veles goal list
veles goal resume <id>    # продолжить после паузы или Ctrl+C
```
...
| `Ctrl+X Ctrl+E` | Открыть `$EDITOR` для текущего черновика |
...
| `veles goal {list,show,start,pause,resume,cancel}` | Долгосрочные цели |
