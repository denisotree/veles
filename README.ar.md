...
**معياري لا متجانس** — نواة بسيطة (الذاكرة، حلقة الوكيل، بروتوكول المورّد، سجل الأدوات). وكل ما عداها — الخادم الخفي، وبوابة Telegram، والبحث المعمّق، ومجدول المهام — وحدة اختيارية قابلة للتحميل.
...
| `Ctrl+X Ctrl+E` | فتح `$EDITOR` على المسوّدة الحالية |
...
| `veles goal {list,show,start,pause,resume,cancel}` | الأهداف طويلة الأفق |
